...
int main(void)
{
	...
	if (0)
	{
		...
		mtsr606_KickSetState(MTSR_STATE_START, (float)(FREQUENCY), 0.5f, VEL_VOL_MOD, VEL_TONE_MOD, REFERENCE_VEL, &s);
		mtsr_TailSetProgram((float)(FREQUENCY), 10.0f, &tail_p);
		mtsr_TailSetState(&tail_s);

		memset(s_buffer, 0, sizeof(float) * BUFFER_LEN);
		const float last_signal =
		    mtsr606_RenderKick(1.0f, &p, &s, s_buffer, s_buffer + 2460); // Fewer samples, cut in the middle

		mtsr_TailAccumulate(&tail_s, last_signal);

		if (1)
		{
			for (size_t i = 2460; i < BUFFER_LEN / 4; i += 1)
				s_buffer[i] = mtsr_TailStep(&tail_p, &tail_s);
		}
	...
	if (0)
	{
		...
		mtsr606_SnareSetState(MTSR_STATE_START, (float)(FREQUENCY), 666, 0.5f, VEL_VOL_MOD, VEL_TONE_MOD, REFERENCE_VEL,
		                      &s);
		mtsr_TailSetProgram((float)(FREQUENCY), 10.0f, &tail_p);
		mtsr_TailSetState(&tail_s);
		...
		memset(s_buffer, 0, sizeof(float) * BUFFER_LEN);
		const float last_signal =
		    mtsr606_RenderSnare(1.0f, &p, &s, s_buffer, s_buffer + 910); // Fewer samples, cut in the middle
