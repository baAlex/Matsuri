...
static bool sPluginParametersValue(const clap_plugin_t* _plugin, clap_id id, double* value)
{
...
static void sPluginParametersFlush(const clap_plugin_t* _plugin, const clap_input_events_t* in,
...
static bool sPluginInitialise(const struct clap_plugin* _plugin)
{
	struct MatsuriPlugin* plugin = (struct MatsuriPlugin*)(_plugin->plugin_data);
...
static void sPluginDestroy(const struct clap_plugin* _plugin)
{
	struct MatsuriPlugin* plugin = (struct MatsuriPlugin*)(_plugin->plugin_data);
	free(plugin);
}

static bool sPluginActivate(const struct clap_plugin* _plugin, double sampling_frequency, uint32_t minimum_frames_count,
...
static void sPluginDeactivate(const struct clap_plugin* _plugin)
{
	(void)_plugin;
}

static bool sPluginStartProcessing(const struct clap_plugin* _plugin)
{
	(void)_plugin;
	return true;
}

static void sPluginStopProcessing(const struct clap_plugin* _plugin)
{
	(void)_plugin;
}

static void sPluginReset(const struct clap_plugin* _plugin)
{
	struct MatsuriPlugin* plugin = (struct MatsuriPlugin*)(_plugin->plugin_data);
	VoiceAllocatorSet(&plugin->allocator, plugin->sampling_frequency, MAX_MAX_ITEMS);
	// TODO, should I set parameters again?, like in Initialise()???
...
static clap_process_status sPluginProcess(const struct clap_plugin* _plugin, const clap_process_t* process)
{
	struct MatsuriPlugin* plugin = (struct MatsuriPlugin*)(_plugin->plugin_data);
...
static void sPluginOnMainThread(const struct clap_plugin* _plugin)
{
	(void)_plugin;
}
